...
### 1. Experience Replay
  ...
  논문에서는 sample data간의 correlation을 없애기 위해 여러 trajectory에서 transition sample들을 **Replay Buffer**라는 방법을 제안합니다.
  Q-learning은 off-policy이므로 다른 policy에서 생성된 data로도 학습이 가능하다는 것이 장점입니다. 그래서 논문에서는 correlation이 낮은 sample을 모으기 위해  
### 2. Target Network
