...
### 2. Target Network
  ...
  우리는 앞에서 봤드시 Loss function을 다음과 같이 나타낼 수 있었습니다.
...
$$
  ...
  여기서 target의 Q-function과 prediction의 Q-function은 동일한 network parameter에 의해 update되기 때문에 학습을 진행하면서 함께 변화하는 moving target문제가 발생합니다. 이를 해결하기 위해 논문에서는 별도의 target network를 하나 더 만들어 일정한 주기로 update해주는 방법을 제안합니다. 이렇게 하면 target의 Q-function을 별도의 parameter $\theta^-$로 주기적으로 update하기 때문에 학습 도중 prediction을 계산하는 과정에서 target이 변하는 문제는 발생하지 않게 됩니다.
