...
### 3. Annealing the bias
  ...
  일반적으로 Non-uniform sampling에는 bias가 발생합니다. PER의 경우 stochastic sampling에 의해 replay memory의 sample 분포와 sampling된 sample 분포간의 차이가 생기게 되고 이는 추정하려는 값에 변화를 일으킵니다. 논문에서는 Importance-sampling(IS) weight $w_i$를 도입하여 이러한 문제를 해결합니다. 기존의 TD-error $\delta$대신 weight을 적용한 $w_i \delta$를 사용하는 것입니다.
